...
class FoundationLMClient:
    ...
    def complete(self, prompt: str, max_tokens: int = 256, **kwargs) -> str:
        ...
        response = httpx.post(
            f"{self.base_url}/v1/completions", json=payload, headers=self.headers, timeout=60
        )
    ...
    def chat(self, messages: list[dict], **kwargs) -> str:
        ...
        response = httpx.post(
            f"{self.base_url}/v1/chat/completions", json=payload, headers=self.headers, timeout=60
        )
        response.raise_for_status()
