...
class TrainingEngine:
    def __init__(self, model: VajraForCausalLM, config: TrainingConfig):
        ...
        self.checkpoint_manager = TrainingCheckpointManager(self.config.output_dir, self.config.save_total_limit)
    ...
    def train(self, resume_from_checkpoint: Optional[str | Path] = None):
        ...
        while self.global_step < self.config.max_steps:
            ...
            dt = t1 - t0
            tokens_per_sec = (self.config.batch_size * self.config.max_sequence_length * self.config.gradient_accumulation_steps) / dt
