...
class ProductionTrainingEngine:
    ...
    def train_step(self, batch: torch.Tensor) -> dict:
        ...
        if self.config.fault_tolerance.enable_watchdog and not self.watchdog.check_loss(loss.item()):
        ...
        if self.config.fault_tolerance.enable_watchdog and not self.watchdog.check_gradients(self.model):
            if self.config.fault_tolerance.skip_nan_gradients:
