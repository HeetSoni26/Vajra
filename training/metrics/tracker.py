...
class MetricsTracker:
    ...
    def __init__(self, output_dir: str | Path, log_interval: int = 10):
        ...
        if not self.csv_path.exists():
            with open(self.csv_path, "w", newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    "step", "loss", "learning_rate", "grad_norm", 
                    "tokens_processed", "samples_processed", "tokens_per_sec", 
                    "step_time", "epoch"
                ])
    ...
    def log(self, step: int, metrics: Dict[str, Any]):
        ...
        # Log to Console
        print(f"Step {step} | Loss: {metrics.get('loss', 0.0):.4f} | LR: {metrics.get('learning_rate', 0.0):.2e} | "
