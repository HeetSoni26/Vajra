...
class TrainingCheckpointManager:
    ...
    def _rotate_checkpoints(self):
        checkpoints = [d for d in self.output_dir.iterdir() if d.is_dir() and d.name.startswith("checkpoint-") and d.name != "checkpoint-best"]
    ...
    def load_checkpoint(
        ...
        loaded_model = ModelCheckpointManager.load_checkpoint(checkpoint_dir, device=str(next(model.parameters()).device))
        ...
        if scheduler and (checkpoint_dir / "scheduler.pt").exists():
            scheduler.load_state_dict(torch.load(checkpoint_dir / "scheduler.pt", weights_only=True))
