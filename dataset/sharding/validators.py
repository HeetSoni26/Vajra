        ...
        try:
            ...
            for idx, seq in enumerate(reader.stream()):
                if len(seq) != reader.metadata.sequence_length:
                    results["valid"] = False
                    results["errors"].append(f"Sequence {idx} length mismatch. Expected {reader.metadata.sequence_length}, got {len(seq)}.")
                    
                # 3. Token range validation
                max_token = seq.max() if len(seq) > 0 else 0
                if max_token >= reader.metadata.vocab_size:
                    results["valid"] = False
                    results["errors"].append(f"Sequence {idx} contains out-of-bounds token {max_token} for vocab size {reader.metadata.vocab_size}.")
