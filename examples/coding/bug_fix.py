...
def main():
    responses = ["Diagnosed bug in calc.py. Fixed division by zero check and verified with pytest."]
    reasoner = MockReasoner(responses)
    agent = FoundationAgent(reasoner)

    wf = BugFixWorkflow(agent)
    res = wf.execute(
        bug_description="ZeroDivisionError when dividing by 0 in calc.py", file_target="calc.py"
    )
