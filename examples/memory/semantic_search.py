...
def main():
    mem = MemoryManager()
    mem.remember(
        "Implemented KV Cache acceleration in inference/kv_cache.py", metadata={"area": "inference"}
    )
    mem.remember(
        "Configured Distributed Data Parallel (DDP) launch scripts in training/",
        metadata={"area": "training"},
    )
    mem.remember(
        "Built FastAPI endpoints for completion and tokenization in api/main.py",
        metadata={"area": "api"},
    )
