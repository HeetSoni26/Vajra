...
def test_clean_document():
    # HTML removal
    assert clean_document("<p>Hello world this is a test document with enough length to pass the filter.</p>") == "Hello world this is a test document with enough length to pass the filter."
...
def test_prepare_huggingface(mock_load_dataset, mock_tokenizer_class, tmp_path):
    mock_ds = [
        ...
        {"text": "Too short"},
        {"text": "This is a duplicate English document that is definitely long enough to pass the min length check.", "language": "en"},
        {"text": "This is a duplicate English document that is definitely long enough to pass the min length check.", "language": "en"},
        {"text": "Ceci est un document en français qui est assez long pour passer.", "language": "fr"},
    ]
