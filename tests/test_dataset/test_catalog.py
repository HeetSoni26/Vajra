...
def sample_datasets():
    ds1 = DatasetMetadata(
        name="dataset1", version="1.0.0", description="desc",
        source="local", download_method="local", license="Apache 2.0",
        domain="code", format="jsonl", dataset_type=DatasetType.PRETRAINING,
        quality_rating=QualityRating.HIGH, estimated_size_bytes=100
    )
    ds2 = DatasetMetadata(
        name="dataset2", version="1.0.0", description="desc",
        source="local", download_method="local", license="CC-BY-NC",
        domain="math", format="parquet", dataset_type=DatasetType.INSTRUCTION,
        quality_rating=QualityRating.MEDIUM, tags=["reasoning"]
    )
    return [ds1, ds2]
...
def test_scoring_framework(sample_datasets):
    ...
    assert QualityScoringFramework.map_score_to_rating(ds1_scores["total_score"]) == QualityRating.HIGH
